...
namespace uiuc {
  ...
  void PNG::crop(unsigned int ULX, unsigned int ULY ,unsigned int DRX, unsigned int DRY){
  //Calculate the newWidth and height of crop image
    unsigned int newWidth, newHeight;
    //std::cout << "ULX,ULY,DRX,DRY: " << ULX << ","<< ULY << ","<< DRX << ","<< DRY << ","<<std::endl;
    if(ULX <= DRX && ULY <= DRY){ //check wrong input values
      newWidth = DRX - ULX;
      newHeight = DRY - ULY;
      //std::cout << "newWidth, newHeight " << newWidth <<"," << newHeight << std::endl;
      // Create a new vector to store the image data for the new (crop) image
      HSLAPixel * newImageData = new HSLAPixel[newWidth * newHeight];
      
      // Copy the current data to the new image data, using the existing pixel
      // for coordinates within the bounds of the old image size
      for (unsigned int x = ULX; x < DRX; x++) {
        for (unsigned int y = ULY; y < DRY; y++) {
          if (x < width_ && y < height_) {
            HSLAPixel & oldPixel = this->getPixel(x, y);
            HSLAPixel & newPixel = newImageData[ (x-ULX + ((y-ULY) * newWidth)) ]; //start new image in (0,0)
            newPixel = oldPixel;
          }
        }
      }
